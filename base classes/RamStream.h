...
// CRamBlockStream
class CRamBlockStream : public CBlockStream
{
public:
	CRamBlockStream();
	CRamBlockStream(const CRamBlockStream &rhs);
	~CRamBlockStream();
	CRamBlockStream &operator=(const CRamBlockStream &rhs);

	bool			isvalid() const;
	void			clear();
	void			setbuffer(const void *buffer, int buffersize);
    ...
    using CBlockStream::Read;
	INT64			PhysicalLength() const;
protected:
	void			initfields();
	void			clearfields();
	void			assignfields(const CRamBlockStream &rhs);
	void			assign(const CRamBlockStream &rhs);

	char*			m_buffer;
	int				m_buffersize;
private:
	typedef CBlockStream inherited;
};
